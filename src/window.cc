...
void Window::run(int pacmangrid, int pacmandir, int pacstep, int redgrid, int pinkgrid, int bluegrid, int orangegrid)
{
    //sf::RenderWindow window(sf::VideoMode(500, 500), "SFML works!");
...
//renderiza os 4 energizadores
    ...
    draw_pacman(((int) clock.getElapsedTime().asMilliseconds()/200) % 3 , (pacmangrid%28)*16-15,(pacmangrid/28)*16, 0, &window);
    draw_ghost(((int) clock.getElapsedTime().asMilliseconds()/200) % 2 , 245, 150, 0, pink_ghost_animate, &window);
    draw_ghost(((int) clock.getElapsedTime().asMilliseconds()/200) % 2 , 150, 150, 0, blue_ghost_animate, &window);
